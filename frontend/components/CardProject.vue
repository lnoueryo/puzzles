  ...
  <div class="card-frame" v-if="isReadyObj(project)">
    <v-row class="py-8 relative" align="center" justify="center">
      ...
      <v-btn
        ...
        right
        color="#295caa"
        v-if="projectAuthority.type.name == '管理者'"
        :to="{ name: 'project-id-edit', params: { id: $route.params.id } }"
      >
    ...
    <v-row class="py-8" align="center" justify="center">
      ...
      <v-list subheader two-line>
        ...
        <v-list-item
          ...
          :key="authUser.id"
          @click="$router.push({ name: 'profile-user_id', params: { user_id: authUser.user_id } })"
        >
          <v-list-item-avatar>
            <v-img
              :src="$config.mediaURL + '/users/' + authUser.user.image"
              v-if="authUser.user.image"
            >
              <template v-slot:placeholder>
                <v-row class="fill-height ma-0" align="center" justify="center">
                  <v-progress-circular indeterminate color="grey lighten-5" />
            ...
            <v-menu right bottom>
              ...
              <v-list>
                <v-list-item
                  ...
                  :key="i"
                  @click.native="option.event($event, authUser)"
        ...
        <v-card>
          ...
          <v-card-text>
            <v-radio-group v-model="selectedUser" column>
              <v-radio
                :value="auth"
                v-for="(auth, i) in unregisteredUsers"
                :key="i"
                color="#295caa"
              >
                <template v-slot:label>
                  ...
                  <div v-else>
                    {{ auth.user.email }}　<strong class="red--text"
                      >招待中</strong
                    >
                  </div>
                </template>
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              color="#295caa"
              @click="onAddUser"
              :disabled="isEmptyObj(selectedUser)"
            >
              変更
            </v-btn>
          </v-card-actions>
...
export default Vue.extend({
  ...
  computed: {
    ...
    unregisteredUsers() {
      return this.organizationAuthority.organization.users.filter(
        (oUser: model.OrganizationAuthority) => {
          return !this.project.authority_users.some(
            (user: model.ProjectAuthority) => user.user_id == oUser.user_id
          );
        }
      );
    },
    ...
    projectImage() {
      return this.projectImageError
        ? require("~/assets/image/project.png")
        : this.$config.mediaURL + "/projects/" + this.project.image;
    },
  },
  methods: {
    ...
    async onChangeAuthority() {
      const selectedUser = {
        ...this.selectedUser,
        ...{ auth_id: this.changeAuthority.id, type: this.changeAuthority },
      };
      let response;
      try {
        response = await this.$store.dispatch(
          "project/updateProjectAuthority",
          selectedUser
        );
      } catch (error: any) {
    ...
    async onDeleteAuthority() {
      ...
      try {
        response = await this.$store.dispatch(
          "project/deleteProjectAuthority",
          this.selectedUser
        );
      } catch (error: any) {
        response = error.response;
